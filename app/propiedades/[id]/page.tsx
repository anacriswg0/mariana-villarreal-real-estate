...
export default async function PropertyDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="property-detail">
    ...
    {property.unitTypes&&<section className="detail-unit-types"><h2>Tipologías de penthouse</h2><p>Superficies interiores y exteriores según el folleto del proyecto.</p><div className="detail-unit-grid">{property.unitTypes.map(unit=><article key={unit.name}><h3>{unit.name}</h3><dl><dt>Interior</dt><dd>{unit.interior.toFixed(2)} m²</dd><dt>Exterior</dt><dd>{unit.exterior.toFixed(2)} m²</dd><dt>Total</dt><dd>{unit.total.toFixed(2)} m²</dd></dl></article>)}</div></section>}
